...
Za³o¿eniem naszego projektu by³o stworzenie aplikacji z baz¹ danych, która ma s³u¿yæ w Parku Wodnym Aquadrom w Rudzie Œl¹skiej. Program ma zawieraæ funkcjonalnoœci u³atwiaj¹ce pracownikom Parku Wodnego wykonywanie swojej pracy, tzn:
\begin{itemize}
\item Przechowywanie i zarz¹dzanie danymi pracowników,
\item Obs³uga harmonogramu godzin pracy,
\item Tworzenie i zarz¹dzanie notatkami ze zdarzeñ.
\end{itemize}
...
Przyjêliœmy metodykê pracy wzorowan¹ na SCRUM'ie -- co tydzieñ lub dwa dostarczaliœmy nowych funkcjonalnoœci. Ponadto projekt by³ stale na bie¿¹co konsultowany.
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.45]{trello.jpg}
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.7]{wykresy.jpg}
\caption{Statystyka}
\label{git}
\end{center}
\end{figure}
...
\end{lstlisting}
        Zwraca datê i godzinê dla danej komórki na podstawie nag³ówka i wprowadzonej godziny rozpoczêcia lub zakoñczenia pracy.
\begin{lstlisting}
...
\end{lstlisting}
U¿ywaj¹c gotowej funkcji haszuj¹cej dla stringa podanego w jej argumencie uzyskujemy 256 bitowy skrót SHA – wszystkie has³a przetwarzane w programie s¹ w postaciach niejawnych i tak te¿ porównywane s¹ ich zgodnoœci.
...
\subsection{Testy}
Poprawnoœæ dzia³ania aplikacji zosta³a przetestowana za pomoc¹ testów automatycznych (przy u¿yciu pakietu NuGet). W trakcie powstawania programu by³y na bie¿¹co przeprowadzane testy manualne. Przed oddaniem projektu do oceny zosta³y przeprowadzone ostateczne testy manualne, potwierdzaj¹ce poprawnoœæ zachowañ aplikacji.

\subsection{Bezpieczeñstwo danych i aplikacji}
Has³a u¿ytkowników przechowywane w bazie danych programu s¹ hashowane przy u¿yciu funkcji hashuj¹cej SHA-256.\\

Aplikacja poprzez walidacjê wprowadzanych danych zosta³a uodporniona na ataki typu SQL injection.

\section{Podsumowanie} %%%%%%%%%%%%%%%%%%%%%%%%%% Wnioski
Wszystkie za³o¿enia projektu zosta³y spe³nione. Praca w grupie umo¿liwi³a nam zapoznanie siê z~aspektem tworzenia oprogramowania w zespole programistów. Niewielkie problemy z jakimi spotkaliœmy siê podczas realizacji projektu zosta³y sprawnie rozwi¹zane, dziêki dobrej wspó³pracy w grupie, a tak¿e dziêki naszemu repozytorium i jego obs³udze w programie SourceTree. Wspólnymi si³ami osi¹gnêliœmy zamierzony cel -- dostarczyliœmy aplikacjê, która mo¿e byæ z powodzeniem u¿ywana w Parku Wodnym Aquadrom w Rudzie Œl¹skiej.
